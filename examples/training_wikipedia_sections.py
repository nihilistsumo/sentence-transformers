...
def train(triplet_data_dir, output):
    ...
    dev_data = SentencesDataset(examples=triplet_reader.get_examples('validation.csv', 10000), model=model)
...
def main():
    parser = argparse.ArgumentParser(description='Train wiki section embedder as proposed by Dor et al.')
    parser.add_argument('-td', '--triplet_data_dir', help='Path to triplet dataset directory')
    parser.add_argument('-od', '--outdir', help='Path to output directory to store the model')
    args = vars(parser.parse_args())
    triplet_dir = args['triplet_data_dir']
    outdir = args['outdir']
    train(triplet_dir, outdir)
